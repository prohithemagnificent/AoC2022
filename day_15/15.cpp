...
int main(){
    ...
    // check only the line y = 2 000 000
    const int y_TOCHECK = 2000000;
    std::unordered_map<int,bool> impossible;  // true if impossible to be there


    // fill map with impossible positions
    const int x_LOWER = -20000000;
    const int x_UPPER = 20000001;

    for (int x_TOCHECK = x_LOWER; x_TOCHECK < x_UPPER; x_TOCHECK++){
        for (int i = 0; i < sensors.size(); i++){
            int distance = std::abs(sensors[i].first - beacons[i].first) + std::abs(sensors[i].second - beacons[i].second);
            int distance_TOCHECK = std::abs(sensors[i].first - x_TOCHECK) + std::abs(sensors[i].second - y_TOCHECK);
            
            if (distance_TOCHECK <= distance){
                impossible[x_TOCHECK] = true;
            }
        }
        if (x_TOCHECK % 100000 == 0){
            std::cout << "x: " << x_TOCHECK << std::endl;
        }
    }


    // remove actual beacons from map
    for (int i = 0; i < beacons.size(); i++){
        if (beacons[i].second == y_TOCHECK){
            int xToRemove = beacons[i].first;
            impossible[xToRemove] = false;
        }
    }


    int result = 0;
    for (int idx = x_LOWER; idx < x_UPPER; idx++){
        if (impossible[idx]){
            result++;
        }
        if (idx % 100000 == 0){
